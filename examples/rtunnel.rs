...
async fn main() {
    ...
    let auth_res = session.authenticate_publickey("jason", kp_ref).await.unwrap();
    println!("=== auth: {}", auth_res);
    let mut channel = session.channel_open_session().await.context("open channel").unwrap();
    channel.tcpip_forward(false, "127.0.0.1", 8989).await.unwrap();
    /*
    ...
    session.disconnect(Disconnect::ByApplication, "", "English").await.map_err(|e|{
        println!("=== {:#?}", e);
    });
